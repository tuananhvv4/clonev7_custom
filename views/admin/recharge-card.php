...
$body = ["title" => __("Recharge Card"), "desc" => "CMSNT Panel", "keyword" => "cmsnt, CMSNT, cmsnt.co,"];
$body["header"] = "\n \n\n";
$body["footer"] = "\n \n";
...
if(isset($_POST["SaveSettings"])) {
    ...
    $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => __("Cấu hình nạp thẻ cào")]);
    ...
    $currentYear = date("Y");
    $currentDate = date("Y-m-d");
    $total_yesterday = (int) $CMSNT->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND `create_date` LIKE '%" . $yesterday . "%' ")["SUM(amount)"];
    ...
    echo $CMSNT->site("card_status") == 1 ? "selected" : "";
    echo "                                                    value=\"1\">ON\n                                                </option>\n                                                <option ";
    echo $CMSNT->site("card_status") == 0 ? "selected" : "";
    ...
    echo format_currency($CMSNT->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND MONTH(create_date) = '" . $currentMonth . "' AND YEAR(create_date) = '" . $currentYear . "' ")["SUM(amount)"]);
    echo "                                        </p>\n                                        <p class=\"mb-0 text-muted\">Tháng ";
    echo date("m");
    echo "</p>\n                                    </div>\n                                    <div class=\"ms-2\">\n                                        <span class=\"avatar text-bg-info rounded-circle fs-20\"><i\n                                                class='bx bxs-wallet-alt'></i></span>\n                                    </div>\n                                </div>\n                            </div>\n                        </div>\n                    </div>\n                    <div class=\"col-xl-6\">\n                        <div class=\"card custom-card\">\n                            <div class=\"card-body\">\n                                <div class=\"d-flex align-items-center\">\n                                    <div class=\"flex-fill\">\n                                        <p class=\"mb-1 fs-5 fw-semibold text-default\">\n                                            ";
    echo format_currency($CMSNT->get_row("SELECT SUM(amount) FROM cards WHERE  `status` = 'completed' AND YEAR(create_date) = " . $currentYear . " AND WEEK(create_date, 1) = " . $currentWeek . " ")["SUM(amount)"]);
    ...
    echo __("Tổng nạp:");
    echo "                                                <strong\n                                                    style=\"color:red;\">";
    echo format_currency($CMSNT->get_row(" SELECT SUM(`amount`) FROM `cards` WHERE " . $where . " AND `status` = 'completed' ")["SUM(`amount`)"]);
    echo "</strong>\n                                                | ";
    echo __("Thực nhận:");
    echo "                                                <strong\n                                                    style=\"color:blue;\">";
    echo format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `cards` WHERE " . $where . " AND `status` = 'completed' ")["SUM(`price`)"]);
