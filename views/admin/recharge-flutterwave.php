...
$body = ["title" => __("Recharge Flutterwave"), "desc" => "CMSNT Panel", "keyword" => "cmsnt, CMSNT, cmsnt.co,"];
...
if(isset($_POST["SaveSettings"])) {
    ...
    $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => __("Cấu hình nạp tiền Flutterwave")]);
    ...
    $currentYear = date("Y");
    $currentDate = date("Y-m-d");
    $total_yesterday = (int) $CMSNT->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND  `create_gettime` LIKE '%" . $yesterday . "%' ")["SUM(price)"];
    ...
    echo $CMSNT->site("flutterwave_status") == 1 ? "selected" : "";
    echo "                                                    value=\"1\">ON\n                                                </option>\n                                                <option ";
    echo $CMSNT->site("flutterwave_status") == 0 ? "selected" : "";
    ...
    echo __("1 " . $CMSNT->site("flutterwave_currency_code") . " =");
    echo "</label>\n                                        <div class=\"col-sm-5\">\n                                            <div class=\"input-group\">\n                                                <input type=\"text\" class=\"form-control\"\n                                                    value=\"";
    echo $CMSNT->site("flutterwave_rate");
    echo "\" name=\"flutterwave_rate\"\n                                                    placeholder=\"\">\n                                                <span class=\"input-group-text\">\n                                                    ";
    ...
    echo $CMSNT->site("flutterwave_secretKey");
    ...
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND MONTH(create_gettime) = '" . $currentMonth . "' AND YEAR(create_gettime) = '" . $currentYear . "' ")["SUM(price)"]);
    echo "                                        </p>\n                                        <p class=\"mb-0 text-muted\">Tháng ";
    echo date("m");
    echo "</p>\n                                    </div>\n                                    <div class=\"ms-2\">\n                                        <span class=\"avatar text-bg-info rounded-circle fs-20\"><i\n                                                class='bx bxs-wallet-alt'></i></span>\n                                    </div>\n                                </div>\n                            </div>\n                        </div>\n                    </div>\n                    <div class=\"col-xl-6\">\n                        <div class=\"card custom-card\">\n                            <div class=\"card-body\">\n                                <div class=\"d-flex align-items-center\">\n                                    <div class=\"flex-fill\">\n                                        <p class=\"mb-1 fs-5 fw-semibold text-default\">\n                                            ";
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_flutterwave WHERE `status` = 'success' AND YEAR(create_gettime) = " . $currentYear . " AND WEEK(create_gettime, 1) = " . $currentWeek . " ")["SUM(price)"]);
    ...
    echo __("Paid:");
    echo "                                                <strong\n                                                    style=\"color:red;\">";
    echo format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_flutterwave` WHERE " . $where . " ")["SUM(`price`)"]);
