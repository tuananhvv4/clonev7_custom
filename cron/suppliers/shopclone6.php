...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_shopclone6") < time() && time() - $CMSNT->site("time_cron_suppliers_shopclone6") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_shopclone6' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'SHOPCLONE6' ") as $supplier) {
    $getPrice = curl_get2($supplier["domain"] . "/api/GetBalance.php?username=" . $supplier["username"] . "&password=" . $supplier["password"]);
    $CMSNT->update("suppliers", ["price" => $getPrice], " `id` = '" . $supplier["id"] . "' ");
    $result = curl_get2($supplier["domain"] . "/api/ListResource.php?username=" . $supplier["username"] . "&password=" . $supplier["password"]);
