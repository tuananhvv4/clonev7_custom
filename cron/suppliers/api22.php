...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api22") < time() && time() - $CMSNT->site("time_cron_suppliers_api22") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api22' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_22' ") as $supplier) {
