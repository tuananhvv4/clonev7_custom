...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api14") < time() && time() - $CMSNT->site("time_cron_suppliers_api14") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api14' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_14' ") as $supplier) {
    $result = balance_API_14($supplier["domain"], $supplier["token"]);
    $result = json_decode($result, true);
    if(isset($result["user"])) {
        $CMSNT->update("suppliers", ["price" => format_currency($result["user"]["balance"])], " `id` = '" . $supplier["id"] . "' ");
    }
