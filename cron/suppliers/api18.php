...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api18") < time() && time() - $CMSNT->site("time_cron_suppliers_api18") < 2) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api18' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_18' ") as $supplier) {
    ...
    if(isset($result["Data"])) {
        foreach ($result["Data"] as $api) {
            ...
            $api_desc = NULL;
            $api_price = (int) check_string($api["PriceUsd"] * 25000);
            $ck = $api_price * $supplier["discount"] / 100;
            $price = (int) check_string($api["PriceUsd"] * 25000);
