...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api6") < time() && time() - $CMSNT->site("time_cron_suppliers_api6") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api6' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_6' ") as $supplier) {
    $data = balance_API_6($supplier["domain"], $supplier["api_key"]);
    $data = json_decode($data, true);
    if(isset($data["balance"])) {
        $CMSNT->update("suppliers", ["price" => format_currency($data["balance"])], " `id` = '" . $supplier["id"] . "' ");
    }
