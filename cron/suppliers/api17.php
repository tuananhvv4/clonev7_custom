...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api17") < time() && time() - $CMSNT->site("time_cron_suppliers_api17") < 2) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api17' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_17' ") as $supplier) {
    ...
    if($result["status"] == "success") {
        foreach ($result["data"] as $api) {
            ...
            $api_price = (int) check_string($api["money"]);
