...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api19") < time() && time() - $CMSNT->site("time_cron_suppliers_api19") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api19' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_19' ") as $supplier) {
    ...
    if(isset($result["data"])) {
        foreach ($result["data"] as $api) {
            ...
            $api_stock = (int) check_string($api["quality"]);
            $api_desc = NULL;
