...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api4") < time() && time() - $CMSNT->site("time_cron_suppliers_api4") < 5) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api4' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_4' ") as $supplier) {
    $result = balance_API_4($supplier["domain"], $supplier["username"], $supplier["password"]);
    $result = json_decode($result, true);
    if(isset($result["data"]["Data"]["userDetail"]["coin"])) {
        $CMSNT->update("suppliers", ["price" => format_currency($result["data"]["Data"]["userDetail"]["coin"]), "token" => $result["data"]["Data"]["accessToken"]], " `id` = '" . $supplier["id"] . "' ");
    }
    ...
    if(isset($result["data"])) {
        foreach ($result["data"] as $api) {
            ...
            $api_stock = (int) check_string($api["totalProduct"]);
